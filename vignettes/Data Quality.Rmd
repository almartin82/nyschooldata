...
Overall, [data.nysed.gov](http://data.nysed.gov/) does a lot of things right, and I appreciate the general spirit of data accessibility that is happening there.
...
dirty_random <- ex %>% dplyr::slice(
  runif(10, min = 1, max = ex %>% nrow()) %>% round(0)
) 
...
- none of the statewide aggregates have any sort of numeric identifier.  That means that matching year over year would depend on that `name` string - which is always unreliable
...
Here's the output of `clean_assess_db` from this package (`fetch_assess_db` is a wrapper around that function):
```{r}
...
clean_random <- ex2 %>% dplyr::slice(
  runif(10, min = 1, max = ex2 %>% nrow()) %>% round(0)
) 
