...
def get_test_years():
    ...
    if _available_years is None:
        ...
        _available_years = ny.get_available_years()
...
class TestImport:
    ...
    def test_import_functions(self):
        ...
        import pynyschooldata as ny
        assert hasattr(ny, 'fetch_enr')
        assert hasattr(ny, 'fetch_enr_multi')
        assert hasattr(ny, 'tidy_enr')
        assert hasattr(ny, 'get_available_years')
...
class TestFetchEnr:
    ...
    def test_returns_dataframe(self):
        ...
        df = ny.fetch_enr(years['max_year'])
    ...
    def test_total_enrollment_reasonable(self):
        ...
        if 'is_district' in df.columns and 'grade_level' in df.columns:
            ...
            if len(total_df) > 0:
                ...
                # NY should have 2-4 million students
...
class TestFetchEnrMulti:
    ...
    def test_returns_dataframe(self):
        ...
        df = ny.fetch_enr_multi([years['max_year']])
        assert isinstance(df, pd.DataFrame)
    ...
    def test_contains_requested_year(self):
        ...
        df = ny.fetch_enr_multi([test_year])
    ...
    def test_multi_matches_single(self):
        ...
        import pynyschooldata as ny
        years = get_test_years()
        df_single = ny.fetch_enr(years['max_year'])
        df_multi = ny.fetch_enr_multi([years['max_year']])
